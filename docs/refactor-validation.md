...
Using fork revision `512cdd064` ([upstream follow-up PR #712](https://github.com/EKA2L1/EKA2L1/pull/712)) and its Release EKA2L1 simulator executable (SHA-256 `69a0739e76e6f310d5571ada8b654f75dc50b838821b3e6cfcb14e53f94a1a0b`), the original 5320 N-Gage Launcher logged in as account 1 after both `host-tls` and `tls-ca-file` were removed from emulator configuration. It displayed the green online indicator. Enter Text appeared for the focused native editor in the host's upper-right corner, accepted the password, and disappeared on return to Home. The login background remained complete.
...
The same EKA2L1 code passed the default Release simulator regression (12/12), Angry Birds input suite (5/5), Qt build and all three Android ABI builds. The full core run completed 27,297 assertions in 259 cases. Targeted TLS/access-point tests passed 15,876 assertions in 12 cases. Standalone platform-trust probes accepted a valid public certificate, rejected an untrusted public certificate and mismatched hostname, and accepted a self-signed loopback peer with a mismatched name. Apple trust evaluation disables network fetching. Qt/OpenSSL on macOS also completed valid public-chain, untrusted-chain and wrong-hostname handshake probes without a CA file. An additional Android API 36 runtime probe called the new system trust manager: a valid public chain passed, while the self-signed and empty chains were rejected. This verifies the Java trust backend, not a native game UI flow on Android. The [upstream CI run](https://github.com/EKA2L1/EKA2L1/actions/runs/34874131893) passed for the matching PR revision `5b6c4945d`: iOS, Android, Windows, Linux, macOS, sanitizers and dyncom differential tests.

Private IPv4/IPv6 and IPv4-mapped peers also passed the self-signed, mismatched-name handshake unit cases. An additional live probe to the machine's private interface timed out before the temporary TLS server accepted a connection; curl timed out as well. This extra attempt is not counted as an end-to-end LAN pass.
