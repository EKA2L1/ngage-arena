...
Saving the password supplies credentials for the next explicit login; it does not trigger HTTP recovery when a private profile request encounters an expired cookie. With the native checkbox selected, switching online status from Logged Out to Available to Play sends a fresh HTTPS `authenticateUser`, then starts SNAP and authenticated profile requests. This also works after exiting/reopening the Launcher and after a server restart. The inspected Launcher starts offline after a normal exit. Neither the SOAP error response nor the active SNAP identity is used to grant an expired HTTP session access.
